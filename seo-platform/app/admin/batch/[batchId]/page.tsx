...
import { decidePublishLinkBanner, formatBatchKstTime, latestBatchUpdatedAt, summarizeBatchTotals } from "@/lib/batch/batch-view"
...
export default async function BatchDetailPage({
  ...
  // 생성 배치 종료 후 게시로 넘길 수 있는 적격 item (ready·warn_ready).
  // publish pump가 이미 게시한 장소는 item 상태가 그대로여도 places.status가 draft가 아니다 — draft 수로 CTA를 판정한다.
  const publishableItems = items.filter((item) => item.status === "ready" || item.status === "warn_ready")
  let draftPlaceCount = 0
  if (!isPublishRun && !isRunning && publishableItems.length > 0) {
    const { countDraftPlaces } = await import("@/lib/batch/publish-batch-service")
    draftPlaceCount = await countDraftPlaces(publishableItems.map((item) => item.place_id))
  }
  const publishBanner = decidePublishLinkBanner({ isPublishRun, isRunning, publishableStatusCount: publishableItems.length, draftPlaceCount })
  ...
  return (
    <section aria-labelledby="batch-detail-title" className="flex flex-col gap-6">
      ...
      {/* 생성 배치가 끝나면 적격(ready·WARN-ready) 중 아직 draft인 장소만 자동 선택된 게시 화면으로 바로 넘어간다 — 부적격은 게시 화면에서 사유와 함께 제외 표시된다. */}
      {publishBanner.kind === "publishable" ? (
        <section aria-label="게시 연결" className="flex flex-wrap items-center justify-between gap-3 rounded-2xl border border-[var(--accent-primary)]/40 bg-[var(--accent-primary)]/10 p-4">
          <p className="text-sm font-semibold leading-6 text-[var(--accent-primary)]">
            게시 가능한 적격 장소 {publishBanner.draftCount}곳이 준비되었습니다. 부적격(품질 검토·실패)과 이미 게시된 장소는 자동으로 제외됩니다.
          </p>
          <Link
          ...
          >
            적격 {publishBanner.draftCount}곳 일괄 게시 →
          </Link>
        </section>
      ) : null}
      {publishBanner.kind === "auto-published" ? (
        <section aria-label="게시 연결" className="rounded-2xl border border-[var(--accent-primary)]/40 bg-[var(--accent-primary)]/10 p-4">
          <p className="text-sm font-semibold leading-6 text-[var(--accent-primary)]">
            적격 장소 {publishBanner.publishableCount}곳이 이미 자동 게시 완료되었습니다. 이 배치에서 추가로 게시할 장소가 없습니다.
          </p>
        </section>
      ) : null}
