...
export default async function PlacesPage({ params }: PlacesPageProps) {
  ...
  const locationText = [page.region, page.city, page.district, page.address].filter((value) => value !== null).join(" · ")
      ...
      <article className="mx-auto flex max-w-6xl flex-col gap-10 py-12 sm:py-16">
        ...
        <header className="grid gap-6 rounded-3xl border border-[var(--border-default)] bg-[var(--surface-elevated)] p-6 sm:p-8 lg:grid-cols-[1fr_auto] lg:items-end">
          <div className="flex max-w-3xl flex-col gap-4">
            ...
            <h1 className="text-4xl font-bold tracking-[-0.015em] text-[var(--text-primary)] sm:text-5xl">{page.title}</h1>
        ...
        <section className="grid gap-4 md:grid-cols-3" aria-label="장소 요약">
          <SummaryCard label="장소명" value={page.place?.name ?? page.title} />
          <SummaryCard label="업종" value={formatPlaceCategory(page)} />
...
function formatPlaceCategory(page: PublicPageDto): string {
  const category = page.place?.category
  const detailCategory = page.place?.detailCategory
  return [category, detailCategory].filter((value) => value !== null && value !== undefined).join(" · ") || "장소 안내"
}
