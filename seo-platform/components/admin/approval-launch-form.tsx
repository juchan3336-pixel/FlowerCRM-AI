...
export function ApprovalLaunchFormView({
  ...
  return (
    <form action={action} aria-busy={isPending} className="flex flex-col gap-5">
      <section aria-label="승인 후보" className="overflow-hidden rounded-3xl border border-[var(--border-default)] bg-[var(--surface-elevated)]">
        <div className="flex items-center justify-between border-b border-[var(--border-default)] p-5">
          <h3 className="text-lg font-semibold text-[var(--text-primary)]">공식 검증 완료 후보</h3>
        ...
        {candidates.length === 0 ? (
          <p className="p-6 text-sm leading-6 text-[var(--text-secondary)]">
            승인 가능한 후보가 없습니다. 대상은 공식 검증(official_verification_status=verified)이 완료된 draft 장소이며, AI 생성·SEO 페이지가 아직 없어야 합니다.
          </p>
        ) : (
                      ...
                      <td className="px-4 py-3">
                        {candidate.eligible ? (
                          ...
                          <span className="whitespace-nowrap rounded-full border border-[var(--status-warning)]/40 bg-[var(--status-warning)]/10 px-2.5 py-0.5 text-xs font-semibold text-[var(--status-warning)]">
                            불가 — {candidate.reason === null ? "사유 미상" : BATCH_INELIGIBLE_LABELS[candidate.reason]}
                          </span>
                        )}
                      </td>
                    </tr>
