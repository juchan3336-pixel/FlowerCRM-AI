...
import { detectTitlePatternId, pickTitlePattern, TITLE_PATTERNS, titleRegionLabel } from "@/lib/ai/title-variation"
import type { AiGeneratedSeoContent } from "@/lib/ai/types"

const EIGHTH = { placeId: "e690ff19-b2aa-4574-929f-c8371a1b311a", placeName: "영남대학교병원 장례식장", city: "대구", district: "남구" }
const NINTH = { placeId: "692d0927-0ba9-412c-bed8-334be14fd4b7", placeName: "대구파티마병원 장례식장", city: "대구", district: "동구" }
const TENTH = { placeId: "710a371e-e767-4d65-8706-a2b19e66f6dd", placeName: "조은금강병원 장례식장", city: "경남", district: "김해시" }
...
describe("제목 다양화", () => {
  it("defines eight distinct title patterns", () => {
    ...
    expect(TITLE_PATTERNS).toHaveLength(8)
    const titles = TITLE_PATTERNS.map((pattern) => pattern.build("영남대학교병원 장례식장", "대구 남구"))
    expect(new Set(titles).size).toBe(8)
    expect(new Set(TITLE_PATTERNS.map((pattern) => pattern.id)).size).toBe(8)
  })
  ...
  it("avoids patterns used by the recent five pages", () => {
    ...
    for (let index = 0; index < 40; index += 1) {
      const pick = pickTitlePattern(`seed-${String(index)}`, "테스트병원 장례식장", "김해시", recent)
  ...
  it("detects template titles after masking place and region", () => {
    ...
    expect(detectTitlePatternId("영남대학교병원 장례식장 근조화환 주문 안내", "영남대학교병원 장례식장", ["대구 남구"])).toBe("order-guide")
    expect(detectTitlePatternId("김해시 조은금강병원 장례식장 근조화환 주문 체크사항", "조은금강병원 장례식장", ["김해시"])).toBe("region-checklist")
    expect(detectTitlePatternId("완전히 다른 형식의 제목", "조은금강병원 장례식장", ["김해시"])).toBeNull()
  })
  ...
  it("keeps titles within the length budget and builds region labels correctly", () => {
    ...
    const pick = pickTitlePattern("seed", longPlace, "창원시 마산회원구", { patternIds: [], suffixKeys: [] })
...
describe("키워드 다양화", () => {
  it("builds five role-based keywords without the stock trio or banned terms", () => {
    // Given / When: 10호점 조건.
    const plan = buildKeywordPlan({ seed: "s", placeName: TENTH.placeName, city: TENTH.city, district: TENTH.district, faqTopicKeys: ["unknown-room", "branch-lookup"], recentSets: [] })
  ...
  it("rebuilds automatically when overlapping a recent set at 4/5", () => {
    ...
    const base = buildKeywordPlan({ seed: "s", placeName: EIGHTH.placeName, city: EIGHTH.city, district: EIGHTH.district, faqTopicKeys: ["pre-order-check", "unknown-room"], recentSets: [] })
    ...
    const rebuiltPlan = buildKeywordPlan({ seed: "s", placeName: EIGHTH.placeName, city: EIGHTH.city, district: EIGHTH.district, faqTopicKeys: ["pre-order-check", "unknown-room"], recentSets: overlappingRecent })
...
describe("품질 검사 강화", () => {
  ...
  function input(recentPages: readonly RecentContentSnapshot[]): QualityEvaluationInput {
    return { content: CONTENT, placeName: "테스트병원 장례식장", regionTokens: ["경남", "테스트시"], verifiedInternalPaths: new Set(), recentPages }
  }
